...
日期：2026-09-11  
分支：`pd-kvpool`  
范围：当前分支最新 3 个 commit，如何让 **vLLM PD（Prefill/Decode 分离）** 同时拥有 P2P KV 传输和共享 KV Cache Pool。

配套文档：

- 用户开关与示例：`docs/perf/rollout_kv_offload.md`
- 原始设计合同：`docs/superpowers/specs/2026-09-09-kv-cache-pool-design.md`
...
| Commit | 类型 | 实际落地 |
|---|---|---|
| `43b44758` `[vllm] feat: enable Mooncake KV cache pool for PD disaggregation` | 主功能 | 配置模型、master 生命周期、MultiConnector 组装、replica 注入、JSON 落盘、CPU 单测与文档 |
| `8a5bc8f5` `[vllm] fix: bugfix` | 收敛 | 去掉过早 `@ray.remote` 装饰；eviction 参数改为可选；GPU `store_tp_size` 与 LCM 互斥；setup 提到 `LLMServerManager.create()` |
| `7360c159` `[vllm] fix: tighten KV cache pool defaults and NPU nixl remap` | 收口 | `global_segment_size` 默认回到 `4GB`；`lookup_rpc_port` 固定用 `engine_id`；NPU 上 `nixl + pool` 复用已有 `MooncakeConnectorV1` 回退 |

读代码时以 **第三个 commit 之后的 HEAD** 为准。前两个 commit 里短暂出现过的 `lookup_rpc_port` 用户字段、`80GB` 默认值，已经被第三个 commit 删掉或改回。
...
flowchart TB
    ...
    VAL --> SPAWN["_spawn_pd_server()"]
    SPAWN --> BUILD["build_kv_transfer_config()<br/>纯函数"]
...
sequenceDiagram
    ...
    participant Actor as MooncakeMasterActor
    ...
    alt GPU 且开了 offload/SSD
        ...
        Driver->>Actor: get_address()
        Actor-->>Driver: 写回 master.address
    else 新建
        Driver->>Actor: 绑在 driver 节点创建 verl_mooncake_master_{job_id}
        Actor->>Master: Popen mooncake_master --port ...
        Actor->>Master: TCP probe 最多 30s
        Actor-->>Driver: ip:port 写回 DictConfig
    end
...
`lookup_rpc_port` 不再是用户可配端口。第三个 commit 之后，**一律写成该实例的 `engine_id`**（UUID hex）。上游把它当 IPC 后缀用，用来区分同一节点上多个 vLLM 进程的 lookup 通道。
...
也就是说：NPU 用户即使写了 `transfer_backend=nixl`，开 Pool 时仍复用「Ascend 不支持 NixlConnector、回退 MooncakeConnectorV1」这条旧路径，不会把 NIXL 塞进 MultiConnector。GPU 没有这条回退，开 Pool 必须直接配 mooncake。
...
4. `vllm_pd_replica.py` 的 `launch_servers` / `_spawn_pd_server` — nixl remap、端口、env  
